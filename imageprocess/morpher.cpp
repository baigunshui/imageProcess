...
void Image::Warp(int frame_count, string new_image_name, vector<LinePair> pairs)
{
    ...
    for (int x = 0; x < width; x++)
    {
        for (int y = 0; y < height; y++)
        {
            ...
            Vec3b new_scalar; new_scalar.val[0] = (1 - ratio)*left_scalar.val[0] + ratio*right_scalar.val[0];
    ...
    imshow(img_name, new_image); imwrite(img_name,new_image);
    return;
}
...
void Morpher::morphingRun(QString first_image_name, QString second_image_name)
{
    Morpher::first_image_name = first_image_name.toStdString();
    Morpher::second_image_name = second_image_name.toStdString();
    new_image_name = "morph";
    //设置动画的过渡帧数，比如1为50%，3为25%，50%，75%
    frame_count = 3;
    leftImage = imread(Morpher::first_image_name);
    rightImage = imread(Morpher::second_image_name);
    leftImageTmp = leftImage.clone();
    rightImageTmp = rightImage.clone();
    namedWindow("left", 1);
    moveWindow("left", 500, 300);
    namedWindow("right", 1);
    moveWindow("right", 1050, 300);
...
void Morpher::on_mousel(int event, int x, int y, int flag, void* param)
{
    ...
    if (counter % 2 == 0 && pm->counter > 0)
    {
        ...
        if (event == CV_EVENT_LBUTTONUP || event == CV_EVENT_RBUTTONUP)
        {
            ...
            cout <<"P:"<< curLinePair.leftLine.P.x <<'\t'
                << curLinePair.leftLine.P.y << endl;
            cout << "Q:" << curLinePair.leftLine.Q.x << '\t'
                << curLinePair.leftLine.Q.y << endl;
...
void Morpher::main()
{
    first_image_name = "C:/Users/Administrator/Desktop/3.jpg";
    second_image_name = "C:/Users/Administrator/Desktop/4.jpg";
    new_image_name = "morph";
    //设置动画的过渡帧数，比如1为50%，3为25%，50%，75%
    frame_count = 3;
